<section id="jumbotron">
    ...
    <div class="jumbo-text jumbo-sm d-block d-md-none text-center">
        ...
        <div class="jumbo-button">
            ...
            {{-- added route (if user::auth->index/ else->login ) --}}
            @if (Auth::check())
                    <button type="button" class="btn btn-sm"><a href="{{route("users.dashboard")}}">My profile</a></button>
            @else
                <button type="button" class="btn btn-sm"><a href="{{route("login")}}">Create a profile.</a></button>
            @endif
        </div>
    </div>
